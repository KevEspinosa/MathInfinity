...
import { GameLeyendaStyles } from './GameLegendarioStyles';
...
const GameNovato = ({ navigation }: any) => {
    ...
    const [timeSeconds, changeTimeSeconds] = React.useState<number>(60);
    ...
    const onPressAnswer = (value: number) => {
        if (value === answers[currentAnswer].result) {
            ...
            changeTimeSeconds(timeSeconds + 3);
        }
    }
    ...
    const onPressIncorrectAnswer = (value: number) => {
        if (value === answers[currentIncorrectAnswer].result) {
            ...
            changeTimeSeconds(timeSeconds - 5);
        }
    }
    ...
    return (
        <View style={GameLeyendaStyles.HomeContainer}>
            <TouchableOpacity
                style={GameLeyendaStyles.headerContainer}
                onPress={() => navigation.navigate('Home')}>
                <Image source={require('../../../images/icons/arrow.png')} style={GameLeyendaStyles.menuIcon} />
                <Text style={GameLeyendaStyles.headerTitle}>Legendario</Text>
            </TouchableOpacity>
            <View style={GameLeyendaStyles.HomeContainer}>
                ...
                <View style={{ flexDirection: 'row', flex: 0.8, alignItems: 'flex-end' }}>
                    <View style={{ flexDirection: 'column', flex: 0.8, height: 150 }}>
                        <View style={GameLeyendaStyles.OptionBotton}>

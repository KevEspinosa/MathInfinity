...
const GameFasterView = ({navigation}: any) => {
    ...
    const onPressAnswer = (value: number, index: number) => {
        if (value === answers[currentAnswer].result) {
            ...
            changeTimeSeconds(timeSeconds + 8);
        } else {
    ...
    const newScore = () => {
        navigation.navigate('FasterMode');
        fetch(`http://192.168.100.8:3000/scoreFaster?name=${name}&score=${score}`, {
            method: 'POST'
        })
            .then((response) => response.json())
            .catch((error) => {
            ...
            <Overlay isVisible={isVisibleModal}>
                <View style={{padding: 50, alignItems: "center"}}>
                    ...
                    <Button
                        title="Regresar"
                        titleStyle={{fontSize: 25, padding: 10}}
                        onPress={() => navigation.navigate('FasterMode')}
